...
const filePattern = "./client/src/**/*+(i18n-messages).json";
const outputLanguageDataDir = "client/build/locales/";
...
const i18nScanner = () => {
  ...
  const duplicatedTranslations = _.reduce(
    translation,
    (result, value, key) => {
  ...
  if (!hasError) {
    _.forEach(duplicatedTranslations, (value, key) => {
      if (value.length > 1) {
        console.warn(
          `WARN: The translation "${key}" is repeated in the next IDs: [${value.join(
            ", "
          )}]`
        );
      }
    ...
    console.log(
      `Translations file ${outputLanguageDataDir}i18n-en.json successfully created!`
    );
  }
};
